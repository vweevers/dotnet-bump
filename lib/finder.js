...
module.exports = class Finder extends Emitter {
  ...
  readNuspec (nuspecFile, done) {
    if (this.skip('nuspec', nuspecFile)) return done()
  ...
  scanDirectory (dir, done) {
    ...
    glob('{*,config/*}.{sln,csproj,fsproj,json,json5,nuspec}', { cwd: dir, nodir: true, absolute: true }, (err, files) => {
      if (err) return done(err)
